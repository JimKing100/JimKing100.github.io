...
comments: true
---
[1]: https://medium.com/r/?url=https%3A%2F%2Fgithub.com%2FJimKing100%2FSF_Real_Estate_Live (Full GitHub Repo)
...
All the code, data and associated files for the project can be accessed at my [GitHub][1]. The final Colab code for running on the Bokeh server can be found here. A test version of the Colab code skipping the data cleaning and wrangling steps can be found here.
...
Both Widgets work on the same principle - the callback. In the code below, the Widgets pass a 'value' and call a function named update_plot when the .on_change method is called (when a change is made using the Widget).
...
The test code puts this all together and prints out a static map with the ColorBar and HoverTool in the Colab notebook. In order to see the interactive components using Slider and Select you will need to use the Bokeh server in the next steps. (You will need the fiona and geopandas imports to run the code below)
