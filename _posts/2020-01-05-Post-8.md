...
In the above code **city_url** is constructed using the title, city and state.  The html page is then retrieved and stored in the **page** variable.  BeautifulSoup then parses the page into its html components and stores them in the Beautiful soup object **soup** (a nested data structure).
